...
3. Сделать возможность переноса задачи по времени, а не только по дате.
...
stateDiagram-v2
    ...
    Overdue --> [*]
```
Задачи формируются по крону с помощью отправки пост запроса кроном на `/scheduling/daily-cron-hook`, `/scheduling/weekly-cron-hook`, `/scheduling/monthly-cron-hook`. Эти эндпоинты создают задачи соответственно раз в день, раз в неделю и раз в месяц. Дополнительно, просроченные задачи помечаются просроченными и переходят в Overdue состояние.
Задачи создаются в Pending состоянии, из которого они путём патч запроса на `/tasks/tasks/:id` могут перейти в Running, Cancelled или Moved состояния.
Переход из одного состояния в другое требует указания данных в теле запроса. Для каждого случая эти данные могут быть разными. К примеру, для перехода из Pending в Running нужно в теле запроса предоставить время начала выполнения задачи. А при переходе из Running в Finished - время окончания выполнения задачи.
Перенос задачи осуществляется в следующем порядке:
1. Создание новой задачи в Pending состоянии и без ссылки на шаблон;
2. Обновление старой задачи и указание в ней ссылки на новую.
Следующая задача в свою очередь может быть перенесена таким же способом, за счёт чего задачу можно переносить много раз.
Задачу можно перенести только из Pending состояния. Если задача уже начала выполняться - её нужно выполнить до конца.

Каждая задача (кроме перенесённых) имеет ссылку на шаблон. Этот шаблон хранит общее для всех периодических задач название, описание и теги. Шаблон можно включить или отключить. Когда шаблон выключен - он перестаёт учитываться при создании задач кроном.
