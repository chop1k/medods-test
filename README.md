...
stateDiagram-v2
    ...
    Overdue --> [*]
```
Задачи формируются по крону с помощью отправки пост запроса кроном на:
- `/scheduling/daily-cron-hook`;
- `/scheduling/weekly-cron-hook`;
- `/scheduling/monthly-cron-hook`.
Эти эндпоинты создают задачи соответственно раз в день, раз в неделю и раз в месяц. Дополнительно, просроченные задачи помечаются просроченными и переходят в Overdue состояние.
